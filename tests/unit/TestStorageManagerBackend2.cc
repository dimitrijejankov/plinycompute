...
TEST(StorageManager, StorageManagerBackend2) {
  ...
  if(pid != 0) {
    ...
    backEnd.startServer(make_shared<pdb::GenericWork>([&](PDBBuzzerPtr callerBuzzer) {
      ...
      for(uint64_t i = 0; i < numPages; ++i) {
        ...
        // grab a page
        auto page = backEnd.getFunctionality<pdb::PDBStorageManagerInterface>().getPage(std::make_shared<pdb::PDBSet>("set", "db"), i);

        // set all bytes to the character a
        memset(page->getBytes(), 'a', config->pageSize * 1024 * 1024);
      }
      ...
      for (unsigned long i = 0; i < numThreads; i++) {
        ...
        pdb::PDBWorkPtr myWork = make_shared<pdb::GenericWork>([&, i](PDBBuzzerPtr callerBuzzer) {
          ...
          // this will grab the page increment the bytes on the page by j
          for(unsigned long j = 0; j < numPages; ++j) {

            // grab a page
            auto page = backEnd.getFunctionality<pdb::PDBStorageManagerInterface>().getPage(std::make_shared<pdb::PDBSet>("set", "db"), j);

            // must be not null
            EXPECT_NE(page->getBytes(), nullptr);

            // increment each character
            for(int k = 0; k < config->pageSize; k++) {
              ((char*) page->getBytes())[k] += j;
            }
          }
      ...
      for(uint64_t i = 0; i < numPages; ++i) {
        ...
        for(int j = 0; j < config->pageSize; ++j){
          EXPECT_EQ(((char*) page->getBytes())[j], (char)('a' + numThreads * i));
        }
      }
