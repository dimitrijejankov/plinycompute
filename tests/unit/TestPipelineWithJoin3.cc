...
class SillyJoin : public JoinComp<String, int, StringIntPair, String, int> {
 ...
 public:
  ...
  Lambda<bool> getSelection(Handle<int> in1, Handle<StringIntPair> in2, Handle<String> in3, Handle<int> in4) override {
  ...
  Lambda<Handle<String>> getProjection(Handle<int> in1,
    ...
    return makeLambda(in1,
                      ...
                      in3,
                      in4,
                      [](Handle<int> &in1, Handle<StringIntPair> &in2, Handle<String> &in3, Handle<int> &in4) {
                        std::ostringstream oss;
                        oss << "Got int " << *in1 << " and StringIntPair (" << in2->myInt << ", '" << *(in2->myString)
                            << "') and String '" << *in3 << "' and int " << *in4;
...
int main() {
  ...
  for (int i = 0; i < myJoin->getNumInputs(); i++) {
    std::cout << myJoin->getIthInputType(i) << "\n";
  }
  ...
  String myTCAPString =
      "/* scan the three inputs */ \n\
       ...
       \n\
	   /* join */ \n\
	   AlmostDone (a, c, aAndC, d) <= JOIN (LastHashed (hash), LastHashed (a, c, aAndC), DHashed (hash), DHashed (d), 'JoinComp_3', []) \n\
       \n\
       /* and check for equality */ \n\
       AlmostDoneWithA (a, c, aAndC, d, aExtracted) <= APPLY (AlmostDone (a), AlmostDone (a, c, aAndC, d), 'JoinComp_3', 'self_6', []) \n\
       AlmostDoneWithBoth (a, c, aAndC, d, aExtracted, dExtracted) <= APPLY (AlmostDoneWithA (d), AlmostDoneWithA (a, c, aAndC, d, aExtracted), 'JoinComp_3', 'self_7', []) \n\
       AlmostDoneWithEquals (a, c, aAndC, d, bool) <= APPLY (AlmostDoneWithBoth (aExtracted, dExtracted), AlmostDoneWithBoth (a, c, aAndC, d), 'JoinComp_3', '==_8', []) \n\
