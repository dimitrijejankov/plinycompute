...
TEST(PipelineTest, TestBroadcastJoin) {
  ...
  ON_CALL(*partitionedAPageSet, getNextPage(testing::An<size_t>())).WillByDefault(testing::Invoke(
      [&](size_t nodeID) {
        ...
        std::cout<<" When calling getNextPage() in partitionedAPageSet, size of pageQueuesForA for node: "<< nodeID << " is: "<<pageQueuesForA[nodeID]->size_approx()<<std::endl;
  ...
  ON_CALL(*partitionedCPageSet, getNextPage(testing::An<size_t>())).WillByDefault(testing::Invoke(
      [&](size_t nodeID) {
        std::cout << " When calling getNextPage() in partitionedCPageSet, size of pageQueuesForC for node: " << nodeID << " is: " << pageQueuesForC[nodeID]->size_approx() << std::endl;
        // wait to get the page
        PDBPageHandle page;
