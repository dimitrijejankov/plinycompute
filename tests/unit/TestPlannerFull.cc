...
class GeneticAlgorithm {
 public:
  ...
  GeneticAlgorithm(int32_t initPopulationSize,
                   int32_t numNodes,
  ...
  void generate_assignments(genome_t &out) const {
    ...
    for (auto i = 0; i < join_groups.size(); ++i) {
      ...
      auto lhs = join_groups[i].first;
      auto rhs = join_groups[i].second;
...
int main() {
  ...
  std::vector<char> lhs_record_positions;
  lhs_record_positions.resize(num_lhs_records);
  for (int i = 0; i < num_lhs_records; ++i) {
    lhs_record_positions[i] = i % num_nodes;
  }

  std::vector<char> rhs_record_positions;
  rhs_record_positions.resize(num_rhs_records);
