...
int main(int argc, char* argv[]) {
  ...
  pdbClient.createSet<TRABlock>("myData", "TMP");
  ...
  // do the multiply
  {
    ...
    Handle <Computation> readA = makeObject <TensorScanner>("myData", "TMP");
    Handle <Computation> readB = makeObject <TensorScanner>("myData", "C");
    Handle <Computation> join = makeObject <BCMMJoin>();
    join->setInput(0, readA);
    join->setInput(1, readB);
    Handle<Computation> myAggregation = makeObject<BCMMAggregation>();
    myAggregation->setInput(join);
    Handle<Computation> myWriter = makeObject<TensorWriter>("myData", "D");
    myWriter->setInput(myAggregation);
