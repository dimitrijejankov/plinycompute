...
pdb::Handle<pdb::Computation>& LAInitializerNode::evaluate(LAPDBInstance& instance) {
    ...
    instance.getPDBClient().createSet<MatrixBlock>("LA_db", setName);
            ...
            const UseTemporaryAllocationBlock tempBlock{instance.getBlockSize() * 1024 * 1024};
            {
                ...
                try {
                    ...
                    if (writtenBlocks == totalBlocks) {
                        if (!instance.getPDBClient().sendData<MatrixBlock>("LA_db", setName, storeMatrix)) {
                            std::cerr << "Failed to send data to dispatcher server" << std::endl;
                            exit(1);
                        }
                        //instance.getPDBClient().flushData(instance.instanceErrMsg());
...
pdb::Handle<pdb::Computation>& LAPrimaryExpressionNode::evaluate(LAPDBInstance& instance) {
    if (flag.compare("identifier") == 0) {
        query = identifier->evaluate(instance);
        setDimension(identifier->getDimension());
...
void LAStatementNode::evaluateQuery(LAPDBInstance& instance) {
    ...
    if (expression->isSyntaxSugarInitializer()) {
        ...
        if (statementQuery->getOutputType().compare("MatrixBlock") == 0) {
            if (!instance.getPDBClient().createSet<MatrixBlock>("LA_db", outputSetName)) {
                std::cout << "Not able to create set  <" << outputSetName <<"]." << std::endl;
                exit(-1);
            }
            ...
            if (!instance.getPDBClient().createSet<LAMaxElementOutputType>("LA_db", outputSetName)) {
                std::cout << "Not able to create set  <" << outputSetName <<"]." << std::endl;
                exit(-1);
            }
            ...
            if (!instance.getPDBClient().createSet<LAMinElementOutputType>("LA_db", outputSetName)) {
                std::cout << "Not able to create set  <" << outputSetName <<"]." << std::endl;
                exit(-1);
            }
        ...
        if (!instance.getPDBClient().executeComputations({writeSet})) {
            exit(1);
        }
        std::cout << std::endl;
        ...
        if (printQueryResult) {
            ...
            if (statementQuery->getOutputType().compare("MatrixBlock") == 0) {
                auto output_iter = instance.getPDBClient().getSetIterator<MatrixBlock>("LA_db", outputSetName);
                ...
                auto output_iter = instance.getPDBClient().getSetIterator<LAMaxElementOutputType>("LA_db", outputSetName);
                std::cout << "Output Matrix:" << std::endl;
                int count = 0;
                while(output_iter->hasNextRecord()) {
                    std::cout << count << ":";
                ...
                std::cout << "Max Element output count:" << count << "\n";
            } else if (statementQuery->getOutputType().compare("LAMinElementOutputType") == 0) {
                auto output_iter = instance.getPDBClient().getSetIterator<LAMinElementOutputType>("LA_db", outputSetName);
                std::cout << "Output Matrix:" << std::endl;
                int count = 0;
                while(output_iter->hasNextRecord()) {
                    std::cout << count << ":";
                ...
                std::cout << "Min Element output count:" << count << "\n";
            } else {
                std::cerr << "Invalid query output type!" << std::endl;
                exit(1);
            }
