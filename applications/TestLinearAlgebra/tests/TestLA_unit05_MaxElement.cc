 ...
 *                                                                           *
 *****************************************************************************/
#ifndef TEST_LA_05_CC
#define TEST_LA_05_CC


// by Binhang, May 2017
// to test find the max element in a matrix implemented by aggregation;
#include <ctime>
#include <chrono>
...
int main(int argc, char* argv[]) {
    ...
    if (whetherToAddData == true) {
        ...
        pdbClient.registerType("libraries/libLAMaxElementValueType.so");
        pdbClient.registerType("libraries/libLAMaxElementOutputType.so");

        // now, create a new database
        pdbClient.createDatabase("LA05_db");

        // now, create a new set in that database
        pdbClient.createSet<MatrixBlock>("LA05_db", "LA_input_set");
        ...
        if (numOfMb > 0) {
            ...
            for (int num = 0; num < numIterations; num++) {
                ...
                try {
                    ...
                    pdbClient.sendData<MatrixBlock>("LA05_db", "LA_input_set", storeMe);
    ...
    pdbClient.createSet<MatrixBlock>("LA05_db", "LA_max_set");
    ...
    pdbClient.registerType("libraries/libLAMaxElementAggregate.so");
    pdbClient.registerType("libraries/libLAScanMatrixBlockSet.so");
    pdbClient.registerType("libraries/libLAWriteMaxElementSet.so");



    Handle<Computation> myScanSet = makeObject<LAScanMatrixBlockSet>("LA05_db", "LA_input_set");
    Handle<Computation> myQuery = makeObject<LAMaxElementAggregate>();
    myQuery->setInput(myScanSet);

    Handle<Computation> myWriteSet = makeObject<LAWriteMaxElementSet>("LA05_db", "LA_max_set");
    ...
    if (printResult == true) {
        ...
        auto input_iter = pdbClient.getSetIterator<MatrixBlock>("LA05_db", "LA_input_set");
        ...
        auto output_iter = pdbClient.getSetIterator<LAMaxElementOutputType>("LA05_db", "LA_max_set");
    ...
    pdbClient.removeSet("LA05_db", "LA_max_set");
